...
import { X, History, CarFront, Wrench, ChevronRight, User } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { serviceOrderService } from "../services/serviceOrderService";
import { ServiceOrder, Vehicle } from "../db";

interface VehicleProfileOverlayProps {
  isOpen: boolean;
  onClose: () => void;
  vehicle: Vehicle | null;
}

export function VehicleProfileOverlay({ isOpen, onClose, vehicle }: VehicleProfileOverlayProps) {
  ...
  useEffect(() => {
    if (isOpen && vehicle) {
      setLoading(true);
      serviceOrderService.getVehicleHistory(vehicle.id)
        .then(res => {
          setHistory(res);
        })
      ...
      <div className="modal modal-wide" onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", paddingBottom: '15px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <CarFront size={22} color="var(--primary)" />
            <div>
              <h2 style={{ margin: 0 }}>Perfil do Veículo</h2>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <div>
                <h3 style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1rem', marginBottom: '12px', color: 'var(--text)' }}>
                  <CarFront size={18} color="#10b981" /> Detalhes do Veículo
                </h3>
                <div className="grid-2-col" style={{ gap: '12px' }}>
                  <div style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '12px 16px', background: 'white' }}>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Matrícula</div>
                    <div style={{ fontWeight: 700, color: 'var(--text)' }}>{vehicle.plate}</div>
                  </div>
                  <div style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '12px 16px', background: 'white' }}>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Marca e Modelo</div>
                    <div style={{ fontWeight: 700, color: 'var(--text)' }}>{vehicle.brand} {vehicle.model}</div>
                  </div>
                  <div style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '12px 16px', background: 'white' }}>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Ano</div>
                    <div style={{ fontWeight: 700, color: 'var(--text)' }}>{vehicle.year || '-'}</div>
                  </div>
                  <div style={{ border: '1px solid var(--border)', borderRadius: '12px', padding: '12px 16px', background: 'white' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                      <User size={14} /> Proprietário
                    </div>
                    <div style={{ fontWeight: 700, color: 'var(--text)' }}>{vehicle.client_name}</div>
                  </div>
                </div>
              </div>
